...
| 指標 | 成績 | 備註 |
|:----|:----:|:----|
| 最大 decode 速度 | **26.3 tok/s**（一般版，32K） | 約每秒 26 個 token |
| 最大 prompt 速度 | **391 tok/s**（一般版長文本） | 約每秒 391 個 token |
| 典型實戰速度 | **25-26 tok/s** | 一般對話場景 |
| VRAM 使用 | **8.8-9.1 GB** | 留有 3+ GB 餘裕 |
| RAM 使用 | **13-15 GB** | 部分模型在系統記憶體 |
| 載入時間 | **10-20 秒**（GPU） |
| 支援最大 context | **64K 實測通過**，128K 理論可行 |
...
| n_cpu_moe | 28 |
| threads | 8 |
| context | 32768（32K） / 65536（64K 可用） |
| KV cache | q8_0 / q8_0 |
| flash attn | **on（必須）** |
| fit | off |
| 預計 decode | **~26 tok/s（32K）/ ~25 tok/s（64K）** |
